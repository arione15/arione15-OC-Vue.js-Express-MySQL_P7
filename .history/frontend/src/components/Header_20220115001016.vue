<template>
      <v-app-bar fixed color="#FD2D01" dark>
        ...
        <v-toolbar-items>
          <v-btn v-if="$store.state.isUserLoggedIn" text dark @click="logout"><span class="material-icons">logout</span></v-btn>
        </v-toolbar-items>
      </v-app-bar>
</template>
...
methods: {
  logout(){
    ...
    this.$router.push({name: 'Home'}); //tip to avoid redundant navigation to current location error message (it's not a real error!)
  }
}
}
</script>
