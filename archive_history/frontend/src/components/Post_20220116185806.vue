<template>
  <v-container class="d-flex flex-column pa-2">
    ...
    <v-card-title class="white--text mt-8">
      ...
      <v-btn icon>
<span v-if="isLiked" class="material-icons green" @click="likePost(post.id)">thumb_up</span>
      </v-btn>
...
ifLiked(){
this.likes = this.post.Likes.map(like => like.UserId);
this.isLiked=this.likes.includes(this.$store.state.user.id);
}
},
mounted() {
    // try {
