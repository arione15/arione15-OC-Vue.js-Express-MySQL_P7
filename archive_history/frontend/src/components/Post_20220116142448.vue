 ...
 <v-app-bar flat color="rgba(0, 0, 0, 0)">
          ...
          <v-card-title class="white--text mt-8">
            ...
            <p class="ml-3 red">{{ post.User.firstName }} {{ post.User.familyName }}</p>
                  ...
                  <v-card-text class="red">{{ post.title }}</v-card-text>
          ...
          <v-timeline align-top dense>
            <v-timeline-item v-for="comment in postcomments" :key="comment.id" small>
              <div>
                <div class="font-weight-normal">
                  <strong>{{ post.Comments.User.firstName }} {{ post.Comments.User.familyName }}</strong> @{{ comment.createdAt }}
                </div>
                <div>{{ post.Comments.message }}</div>
              </div>
            </v-timeline-item>
          </v-timeline>
