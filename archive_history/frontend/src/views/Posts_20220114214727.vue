              ...
              <div class="card-body">
                <form action class>
                  <div class="d-flex align-items-start flex-column form-group">
                    ...
                    <label for="formFileSm" class="form-label d-flex align-items-start">Ajoutez une vidéo</label>
                    <v-text-field label="YouTube ID" :rules="[required]" v-model="post.youtubeId"></v-text-field>
                  </div>
                ...
                <form action class>
                  <v-row>
                    <v-col v-for="comment of post.Comments" :key="comment.id">
                      <div class="d-flex align-items-start flex-column form-group">
                        <v-img class="white--text align-end album-image" height="50px" width="50px" :src="comment.User.photoUrl"></v-img>
                        <span>{{ comment.User.firstName }}{{ comment.User.familyName }}</span>
                        <div>{{ comment.message }}</div>
                      </div>
                    </v-col>
                    <div class="d-flex panel-footer form-group">
                      <v-text-field required :rules="rules.required" label="Comment" v-model="message" multi-line 
                      ></v-text-field>
                      <v-btn
                        color="light-blue"
                        dark
                        @click="publishComment(post.id)"
                      >Ajouter votre commentaire !!</v-btn>
                    </div>
                  </v-row>
                </form>
              </div>
