            ...
            .then((user) => {
                ...
                bcrypt
                    ...
                    .then((isPasswordValid) => {
                        ...
                        const cookieToken = jwt.sign( //générer le token
                            { userId: user.id },
                            process.env.SECRET_KEY, { expiresIn: "24h" }
                        );
                        const message = `L'utilsateur s'est connecté avec succès !`;
                        new Cookies(req, res).set('jwtCookie', cookieToken, {
                            httpOnly: true,
                            maxAge: parseInt(process.env.MAX_AGE)
                        });
                        res.status(200).json({ message, data: user, cookieToken }); // retourner le token au client
                    });
            })
            .catch((error) => {
